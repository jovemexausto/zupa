...
describe('Zupa API Middleware', () => {
    it('serves /health check correctly', async () => {
        ...
        app.use('/', createZupaApi({
            state: { agentId: 'agent-123', latestAuthQr: null, isOnline: true }
        }));
    ...
    describe('Static Auth Token', () => {
        const authOptions = {
            ...
            state: { agentId: 'agent-1', latestAuthQr: null, isOnline: false }
        };
    ...
    describe('/auth/qr endpoints', () => {
        const app = express();
        app.use('/', createZupaApi({
            state: { agentId: 'test', latestAuthQr: { qr: 'test-qr-data', updatedAt: '2023-01-01T00:00:00.000Z' }, isOnline: false }
        }));
