...
describe('bindTransportInbound — concurrency limiting', () => {
    it('processes messages up to maxConcurrent without overload', async () => {
        ...
        await transport.simulateInbound({ from: 'u1', body: 'msg1', fromMe: false });
        await transport.simulateInbound({ from: 'u2', body: 'msg2', fromMe: false });
    ...
    it('calls onOverload when at max concurrency', async () => {
        ...
        const first = transport.simulateInbound({ from: 'u1', body: 'first', fromMe: false });
        ...
        await transport.simulateInbound({ from: 'u2', body: 'second', fromMe: false });
    ...
    it('inFlightCount decrements even when engine throws', async () => {
        ...
        await transport.simulateInbound({ from: 'u1', body: 'msg', fromMe: false });
    ...
    it('stops accepting new messages after stop()', async () => {
        ...
        const binding = bindTransportInbound({
            transport,
            runInboundEngine: async () => { processed++; }
        });
