...
describe('Zupa Baseline Core Functionality', () => {
    const TEST_USER_FROM = '5511999999999@c.us';
    ...
    it('should complete a baseline text-to-text turn and persist messages', async () => {
        ...
        await runtime.start();
        await runtime.runInbound({ ...DEFAULT_INBOUND, from: TEST_USER_FROM, body: 'Hi', messageId: 'msg-101' });
    ...
    it('should maintain working memory across multiple turns', async () => {
        ...
        // Turn 1
        await runtime.runInbound({ ...DEFAULT_INBOUND, from: TEST_USER_FROM, body: 'My name is Marcus', messageId: 'msg-201' });

        // Turn 2
        await runtime.runInbound({ ...DEFAULT_INBOUND, from: TEST_USER_FROM, body: 'What is my name?', messageId: 'msg-202' });
    ...
    it('should process audio flow end-to-end (STT -> LLM -> TTS)', async () => {
        ...
        await runtime.runInbound({
            ...
            type: 'ptt' as any,
            hasMedia: true,
            downloadMedia: async () => ({ data: Buffer.from('fake-audio'), mimetype: 'audio/ogg', filename: 'voice.ogg' } as any)
        });
