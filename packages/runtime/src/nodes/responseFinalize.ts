...
/**
 * Converts the LLM response into the final outbound message and sends it to
 * the user via the transport layer. Handles finalization strategies (streaming,
 * batched, etc.) and applies response hooks for extensibility.
 *
 * For non-streaming channels, queues the full response. For streaming/UI channels,
 * may relay tokens or interactive updates directly to the client. Also handles
 * tool execution results and iterates the LLM loop if needed.
 *
 * Preconditions: user, session, replyTarget, and llmResponse must be present.
 */
export const responseFinalizeNode = defineNode<
  RuntimeState,
...
>(async (context) => {
  ...
  if (!llmResponse) {
    if (
      ...
      if (enforcer === "voice") prefersVoice = true;
        ...
        prefersVoice = state.inputModality === "voice";
      else if (preference === "dynamic") {
        const hasVoiceReq = /voice|audio|speak|falar|áudio/i.test(
          state.resolvedContent || "",
        );
        const hasTextReq = /text|texto|escreve/i.test(
          state.resolvedContent || "",
        );
      ...
      if (prefersVoice) {
            ...
            retryIdempotent({
              ...
              run: () =>
                resources.llm.complete({
                  messages,
                  systemPrompt: state.builtPrompt!,
        ...
        if (!resolvedResponse)
          throw new Error("Stream finalized without completion response.");
  ...
  const replyText =
    llmResponse.content ||
    (typeof structuredRecord?.reply === "string"
  ...
  if (
    ...
    if (structuredObj.sessionEnded === true && session && user) {
      const summary = `Session ended at ${new Date().toISOString()}`;
      await resources.domainStore.endSession(session.id, summary);
    }
  }
      ...
      if (enforcer === "voice") {
        ...
        if (llmChoice === "voice") {
          ...
          if (customChoice === "voice") {
            ...
            // Heuristic fallback
            const hasVoiceRequest = /voice|audio|speak|falar|áudio/i.test(
              state.resolvedContent || "",
            );
            const hasTextRequest = /text|texto|escreve/i.test(
              state.resolvedContent || "",
            );
  ...
  // 3. Sliding Window Reducer for Working Memory
  const userMsg = { role: "user", contentText: context.inbound.body } as any;
  const assistantMsg = {
    role: "assistant",
    contentText: replyText || "",
  } as any;
  const maxMemory = config.maxWorkingMemory || 20;
  const nextHistory = [...(state.history || []), userMsg, assistantMsg].slice(
    -maxMemory,
  );
