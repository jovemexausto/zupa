 ...
 */
export const responseFinalizeNode = defineNode<RuntimeState, RuntimeEngineContext>(
  async (context) => {
    ...
    const replyText =
      llmResponse.content ||
      (typeof structuredRecord?.reply === "string" ? structuredRecord.reply : undefined);
    ...
    const assistantMsg = { role: "assistant", contentText: replyText || "" } as any;
    const maxMemory = config.maxWorkingMemory || 20;
    const nextHistory = [...(state.history || []), userMsg, assistantMsg].slice(-maxMemory);
