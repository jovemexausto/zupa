...
export interface EngineGraphSpec<TState extends object, TContext = unknown> {
  ...
  nodes: Record<
    string,
    (context: TContext & { state: Readonly<TState> }) => Promise<NodeResult<Partial<TState>>>
  >;
}
...
export interface EngineExecutorConfig<TState = Record<string, unknown>> {
  ...
  entrypoint?: string;
  onStepComplete?: (
    checkpoint: StateSnapshot<TState>,
...
export class EngineExecutor<TState extends object, TContext = unknown> {
  ...
  public async invoke(
    ...
    if (!checkpoint) {
      ...
      const startTasks = config.entrypoint
        ? [config.entrypoint]
        : [Object.keys(this.graph.nodes)[0] as string];
    ...
    let loopCount = 0;
    const MAX_STEPS = (context as any).config?.maxRecursionSteps ?? 50; // Guard against infinite cycles

    while (currentCheckpoint.nextTasks.length > 0) {
      if (loopCount++ > MAX_STEPS) {
        throw new Error(
          `Graph Execution Error: Max steps (${MAX_STEPS}) exceeded for thread ${threadId}`,
        );
      }
      ...
      const snapshotContext = Object.assign({}, context, {
        state: Object.freeze({ ...currentCheckpoint.values }),
      }) as TContext & { state: Readonly<TState> };
