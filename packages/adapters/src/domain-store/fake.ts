import { type Message, type Session, type User, type DomainStore } from "@zupa/core";
...
export class FakeDomainStore implements DomainStore {
  ...
  public async updateUser(
    id: string,
    data: { displayName?: string; preferences?: object },
  ): Promise<void> {
  ...
  public async updateUserPreferences(id: string, prefs: object): Promise<void> {
    return this.updateUser(id, { preferences: prefs });
  }
  ...
  public async countUserMessagesSince(userId: string, since: Date): Promise<number> {
    ...
    return all.filter((m) => m.userId === userId && m.role === "user" && m.createdAt >= since)
      .length;
  }
